...
$task=[pscustomobject]@{task_id='task-test';taskId='task-test';assigned_ai='manus'}
$body=[pscustomobject]@{task_id='task-test';ai='manus';result='done';verdict='PASS';evidence=@('path:artifact.md');acceptance_outcomes=@([pscustomobject]@{criterion='result';status='PASS';evidence='path:artifact.md'});files_changed=@();commit='';tests=@('link PASS');qa=@('mobile PASS');blockers=@();resource_status='AVAILABLE';approval_required=$false;recommended_next_action='AUDIT'}
