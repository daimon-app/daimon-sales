...
function Test-AI5FailureEvidence([string]$Text){
  # Do not turn explicit success evidence such as `is_error: false` into a
  # false failure merely because the field name contains the word "error".
  $normalized=$Text-replace'(?i)\b(?:is_)?error\s*[:=]\s*false\b',''-replace'(?i)\bno\s+errors?\b',''
  $normalized-match'(?i)fail(?:ed|ure)?|error|(^|[^A-Za-z])ng([^A-Za-z]|$)'
}
function Get-AI5ZeroLoopDecision {param($Task,$Result);$null=Initialize-AI5LoopTask $Task;$approvalPending=[bool]$Task.requiresApproval-and$Task.approval.status-ne'approved';if($approvalPending-or$Result.needs_human-or$Result.needsApproval){return [ordered]@{decision='WAITING_APPROVAL';target='';reason='本人承認境界'}};$failText=([string]$Result.failureReason)+' '+(@($Result.tests|Where-Object{Test-AI5FailureEvidence ([string]$_)})-join' ');if($failText.Trim()){Update-AI5LoopGuard $Task $failText|Out-Null;if([int]$Task.loop_state.sameFailureCount-ge3-or[int]$Task.loop_state.dailyExecutionCount-ge10){return [ordered]@{decision='BLOCKED';target='';reason='Loop Guard'}};if($failText-match'(?i)unknown|complex|browser|web|latest|research|資料'){return [ordered]@{decision='REDISPATCH';target=(Get-AI5RedispatchTarget $Task $failText);reason=$failText}};return [ordered]@{decision='REWORK';target=$Task.assignedPrimary;reason=$failText}};[ordered]@{decision='COMPLETE_CANDIDATE';target='codex';reason='必要AI Result受領'}}
function Invoke-AI5DoubleJudge {
  param($Task)
  $null=Initialize-AI5LoopTask $Task;$result=$Task.result;$tests=@($result.tests);$failed=@($tests|Where-Object{Test-AI5FailureEvidence ([string]$_)});$codeWork=$Task.route.workType-eq'code'-or[bool]$Task.route.gitChange;$testEvidence=(!$codeWork-or$tests.Count-gt0)
