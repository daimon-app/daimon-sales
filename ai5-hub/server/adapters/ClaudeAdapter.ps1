...
function Invoke-AI5ClaudeAdapter {
  param($Task)
  $safe=Protect-AI5Object $Task;$primaryResult=if($safe.result){[string]$safe.result.summary}else{'not supplied'}
  $prompt="You are the independent read-only Claude reviewer in AI5 HUB. Verify the objective and the primary AI result. Do not use tools, edit files, run commands, spend money, publish, or request secrets. Start with exactly PASS only when the claimed result is coherent with stated evidence and acceptance criteria; otherwise start with FAIL. Use concise ASCII English. TASK ID: $($safe.taskId) OBJECTIVE: $($safe.objective) ACCEPTANCE: $(@($safe.acceptance_criteria)-join'; ') PRIMARY RESULT: $primaryResult EVIDENCE: $(@($safe.result.evidence)-join'; ') TESTS: $(@($safe.result.tests)-join'; ') QA: $(@($safe.result.qa)-join'; ')"
  try {
    ...
    $verdict=if($summary-match'(?i)^\s*PASS\b'){'PASS'}else{'FAIL'}
