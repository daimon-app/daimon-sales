...
function Get-AI5ZeroLoopDecision {param($Task,$Result);$null=Initialize-AI5LoopTask $Task;if($Task.requiresApproval-or$Result.needs_human-or$Result.needsApproval){return [ordered]@{decision='WAITING_APPROVAL';target='';reason='本人承認境界'}};$failText=([string]$Result.failureReason)+' '+(@($Result.tests|Where-Object{$_-match'(?i)fail|error|ng'})-join' ');if($failText.Trim()){Update-AI5LoopGuard $Task $failText|Out-Null;if([int]$Task.loop_state.sameFailureCount-ge3-or[int]$Task.loop_state.dailyExecutionCount-ge10){return [ordered]@{decision='BLOCKED';target='';reason='Loop Guard'}};if($failText-match'(?i)unknown|complex|browser|web|latest|research|資料'){return [ordered]@{decision='REDISPATCH';target=(Get-AI5RedispatchTarget $Task $failText);reason=$failText}};return [ordered]@{decision='REWORK';target=$Task.assignedPrimary;reason=$failText}};[ordered]@{decision='COMPLETE_CANDIDATE';target='codex';reason='必要AI Result受領'}}
function Invoke-AI5DoubleJudge {
  ...
  $decision=if($Task.requiresApproval){'WAITING_APPROVAL'}elseif($zeroPass){'COMPLETE'}elseif($candidate.decision-eq'REDISPATCH'){'REDISPATCH'}elseif(!$attemptLimitReached-and[int]$Task.loop_state.cycle-lt[int]$Task.loop_state.maxCycles-and$candidate.decision-ne'BLOCKED'){'REWORK'}else{'BLOCKED'}
