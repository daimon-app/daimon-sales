...
function Start-AI5SalesFactory {
  ...
  $created=@();foreach($project in Get-AI5SalesFactoryProjects){
    ...
    $sourceSnapshot=if($DryRun){$null}else{Get-AI5GitHubSourceOfTruthSnapshot -Owner $project.repository.owner -Repository $project.repository.name -Branch $project.repository.activeBranch -ExpectedHead $project.repository.latestCommit}
