...
foreach($entry in $videos.GetEnumerator()){Check (Test-Path $entry.Value) "$($entry.Key) exists";if($ffprobe -and (Test-Path $entry.Value)){$json=& $ffprobe -v error -show_entries format=duration,format_name:stream=codec_name,width,height,r_frame_rate -of json $entry.Value | ConvertFrom-Json;$s=$json.streams[0];Check ($s.width -eq 1080 -and $s.height -eq 1920) "$($entry.Key) resolution";Check ($s.codec_name -eq 'h264') "$($entry.Key) H.264";Check ($s.r_frame_rate -eq '30/1') "$($entry.Key) 30fps"}};Check ([Math]::Abs([double](& $ffprobe -v error -show_entries format=duration -of default=nw=1:nk=1 $videos['CM-A'])-7.0) -lt .1) 'CM-A 7 seconds'
...
$private=Join-Path $repo 'private-sales-workspace'
$aab=Join-Path $private 'app/build/outputs/bundle/release/app-release.aab'
$apk=Join-Path $private 'app/build/outputs/apk/debug/app-debug.apk'
$manifest=Join-Path $private 'app/src/main/AndroidManifest.xml'
$appGradle=Join-Path $private 'app/build.gradle'
$appHtml=Join-Path $private 'app/src/main/assets/index.html'
Check (Test-Path $aab) 'unsigned release AAB exists in ignored private workspace'
Check (Test-Path $apk) 'debug APK exists in ignored private workspace'
Check ((Get-Content -Raw $appGradle).Contains("applicationId 'app.daimon.morning'")) 'provisional applicationId'
...
$saleSource=(Get-Content -Raw $appHtml)
Check (-not ($saleSource -match '(?i)\bWORK\b|\bNIGHT\b')) 'Morning-only source excludes WORK/NIGHT modes'
Check (([regex]::Matches($saleSource,"\['私は")).Count -ge 10) 'Morning 12-message source present'
