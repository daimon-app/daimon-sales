...
try{
  ...
  $sw=Get-Content -Raw (Join-Path $appRoot 'service-worker.js');Assert-PC ($sw-match'fetch'-and$sw-match"const CACHE='ai5-hub-v[0-9]+-[^']+'"-and$sw-match'if\(response.ok\)') 'PWA safe cache version failed'
