...
function Get-AI5CommandCenter {
  ...
  $mobile=Get-MobileHealth
  $remote=[string]$mobile.remote
  $pcState=if(!$bridge.available){'OFFLINE'}elseif($remote-in@('online','serve_active','ready')){'ONLINE'}else{'LOCAL_ONLY'}
  [ordered]@{generatedAt=[DateTime]::UtcNow.ToString('o');pc=$pcState;bridge=$(if($bridge.available){'READY'}else{'OFFLINE'});tailscale=$remote;pixelRoute=$(if($pcState-eq'ONLINE'){'ONLINE'}else{'OFFLINE'});approvalCount=$waiting.Count;agents=$agents;liveTasks=$active;recentResults=@($tasks|Where-Object{$_.status-in@('completed','failed')}|Select-Object -First 12);flows=$flows;writeLocks=@(Get-AI5ActiveLocks);notificationPolicy=[ordered]@{approvalSlots=@('10:00','12:00','15:00');afterHours='18:00';completion='silent_in_app';timeZone='Asia/Tokyo'}}
}
