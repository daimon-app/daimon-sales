...
$workerSource=Get-Content -Raw -Encoding UTF8 (Join-Path $server 'task-service\CodexWorker.ps1');$bridgeSource=Get-Content -Raw -Encoding UTF8 (Join-Path $app 'integrations\codex\zero-codex-bridge\bridge.ps1');if($workerSource-notmatch'UNTRUSTED_WORKDIR'-or$workerSource-notmatch'rev-parse --show-toplevel'-or$workerSource-notmatch'origin does not match signed Project context'){throw'Codex canonical worktree preflight missing'};if($bridgeSource-match'skip-git-repo-check'-or$bridgeSource-notmatch'WorkingDirectory \$workspace'){throw'Bridge trusted-workdir enforcement failed'}
