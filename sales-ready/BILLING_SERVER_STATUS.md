...
- Idempotency, cross-identity replay rejection, timeout and bounded retry
- API, official adapter, RTDN, deployment and recovery documentation

## Production gate

Not deployed. The subscriptions v2 adapter is Fake-tested; before production, replace the memory ledger with durable transactional storage, provision Google/Cloud credentials with Owner approval, configure authenticated Pub/Sub, stage with license testers, and complete security/load review.
