...
foreach ($module in @('router\Router.ps1', 'approval\ZeroApproval.ps1', 'adapters\MockAdapter.ps1', 'storage\Store.ps1', 'security\Security.ps1', 'security\MobileSecurity.ps1', 'task-service\CodexService.ps1', 'orchestrator\TaskEngine.ps1', 'adapters\ClaudeAdapter.ps1', 'adapters\SpecialistRegistry.ps1', 'adapters\NotebookLMAdapter.ps1', 'project-control\ProjectControl.ps1', 'project-control\ProjectRoom.ps1')) {
...
try {
    while ($true) {
        ...
        try {
            ...
            if ($path -match '^/api/projects/([^/]+)/room$' -and $method -eq 'GET') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404}else{Send-Json $stream (Get-AI5ProjectRoom $project)};continue }
            if ($path -match '^/api/projects/([^/]+)/room/instructions$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};$body=Parse-Body $request;try{$r=Add-AI5RoomInstruction $project ([string]$body.text) $(if($session.login){$session.login}else{'TEPPEI'});Send-Json $stream @{project=(Get-AI5ProjectRoom $r.project);instruction=$r.instruction}}catch{Send-Json $stream @{error=$_.Exception.Message} 400};continue }
            if ($path -match '^/api/projects/([^/]+)/pause$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};try{Send-Json $stream (Get-AI5ProjectRoom (Pause-AI5Project $project $(if($session.login){$session.login}else{'TEPPEI'})))}catch{Send-Json $stream @{error=$_.Exception.Message} 409};continue }
            if ($path -match '^/api/projects/([^/]+)/resume$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};try{Send-Json $stream (Get-AI5ProjectRoom (Resume-AI5Project $project $(if($session.login){$session.login}else{'TEPPEI'})))}catch{Send-Json $stream @{error=$_.Exception.Message} 409};continue }
            if ($path -match '^/api/projects/([^/]+)/interrupt$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};$body=Parse-Body $request;Send-Json $stream (Get-AI5ProjectRoom (Set-AI5ProjectInterrupt $project $(if($session.login){$session.login}else{'TEPPEI'}) ([string]$body.text)));continue }
            if ($path -match '^/api/projects/([^/]+)/emergency-stop$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};$body=Parse-Body $request;Send-Json $stream (Get-AI5ProjectRoom (Stop-AI5ProjectEmergency $project $(if($session.login){$session.login}else{'TEPPEI'}) ([string]$body.reason)));continue }
            if ($path -match '^/api/projects/([^/]+)/writer$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};$body=Parse-Body $request;try{Send-Json $stream (Get-AI5ProjectRoom (Set-AI5ProjectWriter $project ([string]$body.writer) $(if($session.login){$session.login}else{'TEPPEI'})))}catch{Send-Json $stream @{error=$_.Exception.Message} 409};continue }
            if ($path -match '^/api/projects/([^/]+)/reviewer$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};$body=Parse-Body $request;try{Send-Json $stream (Get-AI5ProjectRoom (Set-AI5ProjectReviewer $project ([string]$body.reviewer) $(if($session.login){$session.login}else{'TEPPEI'})))}catch{Send-Json $stream @{error=$_.Exception.Message} 400};continue }
            if ($path -match '^/api/projects/([^/]+)/manus-audit$' -and $method -eq 'POST') { $project=Get-AI5Project $Matches[1];if(!$project){Send-Json $stream @{error='not_found'} 404;continue};Send-Json $stream (Get-AI5ProjectRoom (Request-AI5ManusAudit $project $(if($session.login){$session.login}else{'TEPPEI'})));continue }
            if ($method -eq 'GET' -and $path -eq '/api/approvals') { Send-Json $stream (Get-AI5ApprovalAggregate);continue }
