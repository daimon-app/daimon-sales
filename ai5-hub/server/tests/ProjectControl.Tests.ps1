...
try{
  ...
  $p=Set-AI5ProjectAuto $p $true;Assert-PC $p.autoExecution.enabled 'AUTO ON failed';$p=Set-AI5ProjectAuto $p $false;Assert-PC (!$p.autoExecution.enabled) 'AUTO OFF failed'
  ...
  $p.autoExecution.enabled=$true;$p.executionPolicy.consecutiveFailures=2;Assert-PC ((Test-AI5ProjectExecution $p).reason-eq'ERROR_LOOP_STOP') 'ERROR_LOOP_STOP failed'
  ...
  $p.executionPolicy.costClass='NORMAL';$p.executionPolicy.runsToday=3;Assert-PC ((Test-AI5ProjectExecution $p).reason-eq'CREDIT_PROTECTION_STOP') 'daily protection failed'
  $p.executionPolicy.runsToday=0;$p.autoExecution.enabled=$false;Assert-PC ((Test-AI5ProjectExecution $p).reason-eq'AUTO_OFF') 'AUTO OFF execution gate failed'
  ...
  $sw=Get-Content -Raw (Join-Path $appRoot 'service-worker.js');Assert-PC ($sw-match'fetch'-and$sw-match'project-control') 'PWA offline cache version failed'
