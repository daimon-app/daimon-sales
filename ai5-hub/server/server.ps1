...
foreach ($module in @('router\Router.ps1', 'approval\ZeroApproval.ps1', 'orchestrator\ClosedLoopCore.ps1', 'artifacts\ArtifactReturn.ps1', 'adapters\MockAdapter.ps1', 'storage\Store.ps1','storage\Attachments.ps1','storage\LongMessageIngestion.ps1', 'security\Security.ps1', 'security\MobileSecurity.ps1', 'notifications\PushNotification.ps1', 'task-service\CodexService.ps1', 'orchestrator\TaskEngine.ps1', 'orchestrator\ExecutionPolicy.ps1','orchestrator\AutonomousLoop.ps1','orchestrator\TaskQueue.ps1', 'adapters\ClaudeAdapter.ps1', 'adapters\ClaudeApiAdapter.ps1', 'adapters\GeminiApiAdapter.ps1', 'adapters\SpecialistRegistry.ps1', 'adapters\BrowserSpecialistAdapter.ps1', 'adapters\ManusAdapter.ps1', 'adapters\MailManusAdapter.ps1', 'adapters\NotebookLMAdapter.ps1', 'project-control\ProjectControl.ps1','command-center\CommandCenter.ps1','deployment\StableRuntime.ps1','github-bus\GitHubResultLoop.ps1')) {
    . ([ScriptBlock]::Create((Get-Content -Raw -Encoding UTF8 (Join-Path $CodeServerRoot $module))))
}
Initialize-AI5Store $ServerRoot
Initialize-AI5ClosedLoopCore $ServerRoot
Initialize-AI5ArtifactReturn $ServerRoot
...
function New-Task($body, [string]$idem) {
    ...
    $approval = if ($route.requiresApproval) { [ordered]@{ type = $route.approvalType; class=$route.approvalClass; summary = '本人の判断が必要な操作です'; status = 'pending'; deep_link="/?approval=$id"; zero_review = (Get-AI5ZeroApprovalReview $route) } } else { [ordered]@{class='TECHNICAL_AUTO';status='ai_approved';summary='技術判断はAIが検査後に承認します'} }
    ...
    $task=[pscustomobject][ordered]@{
        ...
        requiresApproval = $route.requiresApproval; approval = $approval; approval_evidence=$null; field_mode=$(if($null-ne$body.fieldMode){[bool]$body.fieldMode}else{$true}); field_decision=$null; execution_plan=(Get-AI5ExecutionPlan $route); failure_fingerprints=@(); route = $route; validation=[ordered]@{};artifacts=@();result = $null
...
function Submit-AI5TaskBody($body,[string]$idem) {
    ...
    $task=New-Task $body $idem;$decision=Get-AI5TaskQueueDecision $task @(Get-AI5Tasks 100) @(Get-AI5ActiveLocks)
...
try {
    while ($true) {
        ...
        try {
            ...
            if ($path -match '^/api/tasks/([^/]+)/approve$' -and $method -eq 'POST') { $task = Get-AI5Task $Matches[1]; if (!$task) { Send-Json $stream @{ error = 'not_found' } 404; continue }; if ($task.status -ne 'waiting_approval') { Send-Json $stream @{ error = 'invalid_state' } 409; continue }; $body = Parse-Body $request; if (!(Use-AI5ApprovalToken $task.taskId $body.approvalToken)) { Write-AI5Log 'security' 'approval_rejected' @{ task_id = $task.taskId }; Send-Json $stream @{ error = 'invalid_approval_token' } 403; continue }; $task.approval.status = 'approved';$task.approval_evidence.approval_result='OWNER_APPROVED_AUTO_RESUME';$task.approval_evidence.timestamp=[DateTime]::UtcNow.ToString('o'); Set-AI5TaskStatus $task 'RECEIVED' '本人判断完了・AI5施工を自動再開'; Write-AI5Log 'security' 'approval' @{ task_id = $task.taskId; class=$task.route.approvalClass;login = $session.login }; Send-Json $stream $task; Dispatch-Task $task; continue }
