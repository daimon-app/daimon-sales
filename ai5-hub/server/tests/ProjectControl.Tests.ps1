...
try{
  ...
  $sw=Get-Content -Raw (Join-Path $appRoot 'service-worker.js');Assert-PC ($sw-match'fetch'-and$sw-match'ai5-hub-v56-ja-owner-command-center'-and$sw-match'if\(response.ok\)') 'PWA safe cache version failed'
